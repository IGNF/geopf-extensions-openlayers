...
class HandlebarsPlugin {
    ...
    loadPartials () {
        ...
        this.options.partials.forEach( function (files) {
            ...
            files.forEach( function (file) {
                ...
                console.log("[HandlebarsPlugin] Add partial '"+partialName+"'");
    ...
    dependenciesUpdated (compilation) {
        ...
        const fileNames = Array.from(compilation.fileTimestamps.keys());

        const changedFiles = fileNames.filter((watchfile) => {
            const prevTimestamp = this.prevTimestamps[watchfile];
            const nextTimestamp = compilation.fileTimestamps.get(watchfile);
    ...
    compileAllEntryFiles (compilerOutPutPath, callback) {
        ...
        if ( this.options.entry.path && this.options.entry.pattern ) {
            glob(this.options.entry.pattern, {cwd : this.options.entry.path, nodir : true}, (err, entryFilesArray) => {
                if (err) {
                    console.error("[HandlebarsPlugin] No valid entry -- aborting");
                    throw err;
                }

                if (entryFilesArray.length === 0) {
                    return console.error("[HandlebarsPlugin] No valid entry files found for '"+path.join(self.options.entry.path, this.options.entry.pattern)+"' -- aborting");
                }
            ...
            fs.access( this.options.entry, (err) => {
                if (err) {
                    console.error("[HandlebarsPlugin] Can't access file '"+this.options.entry+"' -- aborting");
                    throw err;
                }
    ...
    compileEntryFile (sourcePath, outputPath, compilerOutPutPath) {
        ...
        if (targetFilePath.includes(compilerOutPutPath)) {
            ...
            this.writeFile(targetFilePath, result, function (err) {
                if (err) {
                    console.error("[HandlebarsPlugin] Can't write file '"+targetFilePath+"'");
                    throw err;
                }
            });
        }

        console.log(`[HandlebarsPlugin] created output '${targetFilePath.replace(`${process.cwd()}/`, "")}'`);
    }
